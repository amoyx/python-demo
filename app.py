...
def mysql():
    ...
    if len(u) > 0:
        for i in u:
            table_name_list.append(i[0])
    return Response("mysql.html", table_name_list=table_name_list, msg='数据库连接成功')


@app.route('/display/env')
def display_env():
    env = {}
    for k, v in os.environ.items():  env[k] = v
    return jsonify(json.dumps(env) if env else {'msg': 'not found environ'})
